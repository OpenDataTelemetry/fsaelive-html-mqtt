...
}).setView([42.0668695, -84.2409325], 17);
...
function changeMap(mapName) {
  (imageUrl = `src/${mapName}.png`), image.setUrl(imageUrl);
}
...
function updateRectangles(rectangle, lat, long) {
  ...
  var bottomRightBound = [
    lat - rectangleSize[0] / 2,
    long + rectangleSize[1] / 2,
  ];
...
function updateTriangles(triangle, lat, long) {
  ...
  var leftPoint = [
    lat - circumcircleRadius[0] / 2,
    long - (circumcircleRadius[1] * Math.sqrt(3)) / 2,
  ];
  var rightPoint = [
    lat - circumcircleRadius[0] / 2,
    long + (circumcircleRadius[1] * Math.sqrt(3)) / 2,
...
function receiveSignal(json) {
  ...
  updatePosition(message.lat, message.lon, topic);
}

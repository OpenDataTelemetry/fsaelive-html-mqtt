...
const gpsSpeedChartInsideText = {
  ...
  afterDatasetsDraw(chart, args, plugins) {
    ...
    const radius =
      generalData[0].innerRadius +
      (generalData[0].outerRadius - generalData[0].innerRadius) / 2;
...
const throttleText = {
  ...
  afterDatasetsDraw(chart, args, plugins) {
    ...
    const word = "THROTTLE";
    const constantAngle =
      (-4.101523742186674 + 0.959931088596881) / 2 - Math.PI / 1.8; // Initial values of generalData[1].startAngle and generalData[1].endAngle
...
const brakeText = {
  ...
  afterDatasetsDraw(chart, args, plugins) {
    ...
    const word = "BRAKE";
    const constantAngle =
      (-0.7330382858376184 + 0.8377580409572779) / 2 - Math.PI / 12; // Initial values of generalData[1].startAngle and generalData[1].endAngle
...
const gpsSpeedData = {
  datasets: [
    {
      data: [0, 300, 60],
      backgroundColor: ["blue", "rgb(0,0,0,0.5)", "rgb(0,0,0,0)"],
      weight: 2,
    },
  ],
};
...
const throttleData = {
  datasets: [
    {
      data: [0, 180, 180],
      backgroundColor: ["green", "rgb(0,0,0,0.2)", "rgb(0,0,0,0)"],
      weight: 2,
      borderWidth: 0.5,
      hoverOffset: 4,
    },
...
const brakeData = {
  datasets: [
    {
      data: [80, 0, 240],
      backgroundColor: ["rgb(0,0,0,0.2)", "red", "rgb(0, 0, 0,0)"],
      weight: 2,
      borderWidth: 0.5,
      hoverOffset: 4,
    },
